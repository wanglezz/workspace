...
>$\beta$函数：
$$</div>\int _0^1x^{a-1}(1-x)^{b-1}dx=\frac{\Gamma(a)\Gamma(b)}{\Gamma(a+b)}<div>$$
...
3. $(A^*)^*=|A|^{n-2}A.$
4. $(AB)^*=B^*A^*.$
> 此结论在$A,B$不可逆时也成立，可用扰动法证得，令$A_1=A+tE,B_1=B+tE$均可逆，则$(A_1B_1)^*=B_1^*A_1^*$，当$t\rightarrow 0$时，根据连续性得原式成立.
...
9. $r(AB)\geq r(A)+r(B)-n.$
10. $A_{m\times n},B_{n\times s},C_{s\times t},则r(ABC)\geq r(AB)+r(BC)-r(B).$
...
>直接令两个方程的通解相等，解出对应系数的关系.
>
>$Ax=b和Bx=c$的公共解
>有公共解的充要条件，$r(A/B)=r(A/B:b/c)$上下拼.
>
...
>
>$Ax=b$和$Bx=c$是同解方程组
>$\Leftrightarrow$齐次方程同 解且非齐次有公共解
