...
4. 若$x=x_0\in I$是函数$f(x)$的唯一可去间断点,则函数$F(x)=\int_a^xf(t)dt$在$x_0$点处不可导,且$F'(x_0)=\lim_{x\rightarrow x_0}f(x)$.
## 反常积分
**无穷区间上反常积分的概念与敛散性**
...
$$</div>y=ux\Rightarrow \frac{dy}{dx}=x\frac{du}{dx}+u,$$于是原方程变为$x\frac{du}{dx}+u=\phi(u)$,即$\frac{du}{\phi(u)-u}=\frac{dx}{x}$.
